...
void World::RunGame()
{
	...
	if(!allMap())
	{
		printf("error: unable to all load maze files!\n");
		return;
	}
	...
	m_graphManager.displayMessageAndWait("Ready to play? Don't cry if you lose");
		...
		switch (RunLevel())
		{
		  case PLAYER_DIED:
			...
			if (m_pcNachMan->GetNumLivesLeft() == 0)
			{
				// game over
				Sleep(250);
				SoundFX::playNachManSound(EPIC_FAIL);
			...
			else
			{
				// you get another try
				Sleep(250);
				SoundFX::playNachManSound(SAD_TROMBONE);
		  ...
		  case FINISHED_LEVEL:
			...
			if (!PrepareLevel())	// load and prepare the next level if there is one
			{
				...
				m_graphManager.displayMessageAndWait("Wow, Eva! Text me this ravenous <3 ");
				gameOver = true;
			}
			else
			{
				...
				Sleep(250);
				SoundFX::playNachManSound(YEAH_BABY);
				const char* msg = "Your current score is: "; 
				char str[30];
				sprintf_s(str, "%s%d", msg, m_pcNachMan->GetScore());
				m_graphManager.displayMessageAndWait(str);
			}
			break;
		}
	} while (!gameOver);
...
bool World::allMap()
{
  ...
  int intStat;
  const char* maze = "maze";
  char filename[10];

  for (int a= 0; a<10; a++)
  {	
	sprintf_s(filename, "%s%d.txt", maze, a);
