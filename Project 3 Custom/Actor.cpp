...
: Actor(w, ITEM_NACHMAN, YELLOW, x, y), m_world(w),  m_life(10), m_score(0), m_power(0)
{}
...
void NachMan::doSomething()
{
	...
	if (getCharIfAny(ch))				{
		switch(ch)		{
			...
			case USE_POWER:		useNachManPower();			break;
			default:	break;
						}				}
...
bool NachMan::allMonsterVuln()
{
	Monster* Monst[NUM_MONSTERS];
	for (int i=0;i<NUM_MONSTERS;i++)
	{
		Monst[i] = m_world->GetMonster(i);
		if (Monst[i]->GetActorState() != VULNERABLE)
			return false;
	}
	return true;
}
...
void Clyde::doSomething()
{
	...
	if (GetActorState() ==VULNERABLE )	{
		int curLevel	= m_world->GetLevel();
		if(GetVulnTicks() == (100 - (curLevel * 10)))
			SoundFX::playNachManSound(SCREAMING);
...
: m_world(w), Monster(w, ITEM_MONSTER2, LIGHTGREEN, x, y), eatyouupCounter(0)
{}
...
bool Stinky::find_n_setNachMan5()
{
	Actor* Nach	= m_world->GetNachMan();
	if (abs(Nach->GetX() - GetX()) <= 5 && abs(Nach->GetY() - GetY()) <= 5)
	{
		if(eatyouupCounter > 5)
		{
			eatyouupCounter = 0;
		    if(!m_world->getSound())
			{
				SoundFX::playNachManSound(MON_EAT_YOU_UP);
				m_world->setSound(true);
			}
		}
		return true;
	}
...
void Inky::InkyDecide()
{
	...
	if (InkChase <= 80 )
	{
		if(!m_world->getSound())
		{	
			SoundFX::playNachManSound(PSYCHO_CHASE);
			m_world->setSound(true);
		}
		ChaseCounter	= 1;
	}
